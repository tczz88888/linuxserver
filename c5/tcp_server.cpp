...
type:tcp用SOCK_STREAM(数据流),udp用SOCK_UGRAM(数据报),2.6.17之后，还可以指定SOCK_NOBLOCK和SOCK_CLOEXEC,分别表示将创建的socket设置非阻塞以及调用fork时，在子进程中关闭该socket
protocol:一般是0
...
int main(int argc,char *argv[]){
    ...
    int ret=bind(sockfd, (struct sockaddr *)&address, sizeof(address));//将sockaddr指向内内容绑定到socket
    assert(sockfd!=-1);
    ...
    if(connfd==-1){
        ...
        printf("connect ip=%s,port=%d\n",inet_ntop(AF_INET, &client.sin_addr, buff, INET_ADDRSTRLEN),ntohs(client.sin_port));
